...
main (int argc, char *argv[])
{
  ...
  NetDeviceContainer switchLowerDevices;
  NetDeviceContainer switchUpperDevices;
  for (int i = 0; i < 4; i++)
    {
      NetDeviceContainer link = csma.Install (NodeContainer (terminals.Get (i), csmaSwitch));
      terminalDevices.Add (link.Get (0));
      switchLowerDevices.Add (link.Get (1));
    }
  ...
  Ptr<ns3::pld::FabricManager> fabric_manager = CreateObject<ns3::pld::FabricManager> ();
  NetDeviceContainer switchLowerDevices;
  NetDeviceContainer switchLowerDevices;
  swtch.Install (switchNode, switchLowerDevices, switchUpperDevices, fabric_manager, EDGE, 0, 0);
