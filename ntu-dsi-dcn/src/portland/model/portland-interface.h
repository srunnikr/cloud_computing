...
private:
    ...
    void FabricManager_ARPRequestHandler(ARPRequest* message) {
        if (isIpRegistered(message->requestedIPAddress)) {
            ...
            FabricManager_ARPResponseHandler(msg);
        } else {
            // Miss in the store, flood it to core
            FabricManager_FloodARPRequest(message);
        }
    }

    void FabricManager_ARPResponseHandler(ARPResponse* message) {
        // Send the message to ARP requestor

    }

    // Packets being generated by the fabric manager
    void FabricManager_FloodARPRequest(ARPRequest* message) {
        // the src packet field contains all required data
    }
