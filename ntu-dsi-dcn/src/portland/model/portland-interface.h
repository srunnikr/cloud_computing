    ...
    void ReceiveFromSwitch (Ptr<PortlandSwitchNetDevice> swtch, BufferData buffer) {
        ...
        switch (packet_type) {
            ...
            case PKT_ARP_RESPONSE:
                ...
                FabricManager_ARPResponseHandler(message, swtch);
                break;
...
private:
    ...
    void FabricManager_FloodARPRequest(ARPRequest* message, swtch, Ptr<PortlandSwitchNetDevice> swtch) {
        for (set<Ptr<PortlandSwitchNetDevice>>::iterator it = m_swtiches.begin(); it != m_switches.end(); it++) {
                SendToSwitch(*it, message, sizeof(message);
        }
        // after broadcasting, try again
        if (isIpRegistered(message->requestedIPAddress)) {
            // IP address present
            ARPResponse* msg = (ARPResponse*) malloc (sizeof(ARPResponse));
            msg->responseMACAddress = getPMACforIP(message->requestedIPAddress);
            ...
            // Can't find the the destination even after broadcasting
            NS_LOG_COND("Destination not found");
        }
